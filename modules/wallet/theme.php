 ...
 * @return
 */
function nv_theme_wallet_withdrawal($row_payment, $post, $array_money_unit,$money_total_info,$list_acountbank)
{
    global $global_config, $module_name, $lang_module, $lang_global, $module_config, $module_info, $op;
    ...
    $xtpl->assign('LANG', $lang_module);
    $xtpl->assign('SRC_CAPTCHA', NV_BASE_SITEURL . "index.php?scaptcha=captcha&t=" . NV_CURRENTTIME);
    $xtpl->assign('TOTALMONEY', number_format($money_total_info['money_total']).' '.$post['money_unit']);
    $xtpl->assign('WITHDRAWAL', number_format($money_total_info['money_total'] - $module_config[$module_name]['minimum']).' '.$post['money_unit']);
    $xtpl->assign('FEES', number_format( $module_config[$module_name]['withdrawal_fees']).' '.$post['money_unit']);
    $xtpl->assign('MIN_MAX', sprintf($lang_module['money_withdrawal_minmax'],$post['money_amount_min'].' '.$post['money_unit'], number_format($money_total_info['money_total'] - $module_config[$module_name]['minimum']).' '.$post['money_unit'] )); 
    $xtpl->assign('NV_BASE_SITEURL', NV_BASE_SITEURL);
    $xtpl->assign('NV_LANG_VARIABLE', NV_LANG_VARIABLE);
    $xtpl->assign('GFX_WIDTH', NV_GFX_WIDTH);
    $xtpl->assign('GFX_HEIGHT', NV_GFX_HEIGHT);
 ...
 * @return
 */
function nv_theme_wallet_transfers($row_payment, $post, $array_money_unit,$money_total_info,$list_user)
{
    global $global_config, $module_name, $lang_module, $lang_global, $module_config, $module_info, $op;
    ...
    $xtpl->assign('TOTALMONEY', number_format($money_total_info['money_total']).' '.$post['money_unit']);
    $xtpl->assign('WITHDRAWAL', number_format($money_total_info['money_total'] - $module_config[$module_name]['minimum']).' '.$post['money_unit']);
    $xtpl->assign('FEES', number_format( $module_config[$module_name]['transfer_fees']).' '.$post['money_unit']);
    $xtpl->assign('MIN_MAX', sprintf($lang_module['money_withdrawal_minmax'],$post['money_amount_min'].' '.$post['money_unit'], number_format($money_total_info['money_total'] - $module_config[$module_name]['minimum']).' '.$post['money_unit'] )); 
    $xtpl->assign('NV_BASE_SITEURL', NV_BASE_SITEURL);
    $xtpl->assign('NV_LANG_VARIABLE', NV_LANG_VARIABLE);
    $xtpl->assign('GFX_WIDTH', NV_GFX_WIDTH);
    ...
    foreach($list_user as $value){
      $xtpl->assign('USER', array(
       'key' => $value['userid'],
       'title' =>$value['username'].' - '.$value['first_name'].' - '.$value['email'],
       'selected' => $post['userid_receiver'] == $value['userid'] ? ' selected="selected"' : ''));
 ...
 * @return
 */
function nv_wallet_acountuser($arr_money_user,$password_wallet)
{
    global $global_config, $module_name, $lang_module, $lang_global, $module_config, $module_info, $op;
 ...
 * @return
 */
function nv_wallet_history_exchange($array,$password_wallet, $generate_page, $page, $per_page)
{
