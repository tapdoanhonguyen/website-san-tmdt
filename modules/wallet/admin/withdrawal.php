...
$db->join('LEFT JOIN ' . $db_config['prefix'] . '_' . $module_data . '_withdrawal tb5 ON tb1.id=tb5.transaction_id LEFT JOIN ' . NV_USERS_GLOBALTABLE . ' tb2 ON tb1.adminid=tb2.userid LEFT JOIN ' . NV_USERS_GLOBALTABLE . ' tb3 ON tb1.userid=tb3.userid LEFT JOIN ' . NV_USERS_GLOBALTABLE . ' tb4 ON tb1.customer_id=tb4.userid ');


if (!empty($where)) {
    $db->where(implode(' AND tb1.status = 2 AND ', $where));
}
else{ $db->where('tb1.status =2');}
$all_page = $db->query($db->sql())->fetchColumn();

$db->order('is_expired ASC, IF(transaction_time=0,created_time,transaction_time) DESC');
$db->limit($per_page);
$db->offset(($page - 1) * $per_page);
$db->select('tb1.*,tb5.*, tb2.username admin_transaction, tb3.username accounttran, tb4.username customer_transaction');
...
while ($row = $result->fetch()) {
    ...
    $arr_list_transaction[$row['id']] = array(
		...
		'acount_name' => $row['acount_name'],
		'acount_cccd' => $row['acount_cccd'],
		'acount_issued_by' => $row['acount_issued_by'],
		'acount_date_range' => $row['acount_date_range'],
		'acount_number' => $row['acount_number'], 
		'acount_bankid' => $row['acount_bankid'], 
		'acount_bankbranch' => $row['acount_bankbranch'], 
...
foreach ($arr_list_transaction as $element) {
    ...
    } elseif ($element['transaction_status'] != 4 and $element['transaction_status'] != 6 and $element['transaction_status'] != 7  and ($IS_FULL_ADMIN or !empty($PERMISSION_ADMIN['is_mtransaction']))) {
        foreach ($global_array_transaction_status as $key => $value) {
           if($element['status'] != 2){$keyid = ($key == 0 or $key == 4  or $key == 7 ) ? ' disabled="disabled"' : '';}
           elseif( $element['status'] != 3){$keyid = ($key == 0 or $key == 4  or $key == 6 ) ? ' disabled="disabled"' : '';}
		   
			else{$keyid = ($key == 0 ) ? ' disabled="disabled"' : '';}
