    ...
    if ($IS_FULL_ADMIN or !empty($PERMISSION_ADMIN['is_mtransaction'])) {
        ...
        if (isset($row['transaction_status']) and $row['transaction_status'] != $new_vid and $row['transaction_status'] != 4 and $new_vid != 0 and empty($row['is_expired'])) {
            ...
            if (!empty($row['order_id'])) {
                ...
                update_money($row['userid'],$row['userid_receiver'], $row['money_total'], $row['money_unit'], $new_vid, $row['transaction_status'], $row['status']);
            }
...
foreach ($arr_list_transaction as $element) {
	if($element['status'] == 1){$element['status_title']=$lang_module['transaction1'];$element['status_money'] = '+';}
	elseif($element['status'] == 2){$element['status_title']=$lang_module['transaction3'];$element['status_money'] = '-';}
	elseif($element['status'] == 3){$element['status_title']=$lang_module['transaction4'];$element['status_money'] = '-';}
	else{$element['status_title']=$lang_module['transaction2'];$element['status_money'] = '-';}
    ...
    if (!empty($element['is_expired'])) {
        ...
        foreach ($global_array_transaction_status as $key => $value) {
			...
			else{$keyid = ($key == 0 or $key == 6 or $key == 7 ) ? ' disabled="disabled"' : '';}
