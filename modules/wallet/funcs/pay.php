...
$order_info = $db->query("SELECT * FROM " . MODULE_WALLET . "_orders WHERE id=" . $order_id)->fetch();
...
if ($nv_Request->isset_request('wallet', 'get')) {
    ...
    $check = $db->exec("UPDATE " . MODULE_WALLET . "_orders SET
        paid_status=4,
...
if ($nv_Request->isset_request('payment', 'get')) {
    ...
    if ($nv_Request->isset_request('wpayportres', 'get')) {
        ...
        $stmt = $db->prepare("SELECT * FROM " . MODULE_WALLET . "_transaction WHERE id = :id");
        ...
        if ($payment != 'vnpay') {
            ...
            $sql = 'UPDATE ' . MODULE_WALLET . '_transaction SET
            ...
            $check = $db->query("UPDATE " . MODULE_WALLET . "_orders SET
    ...
    $transaction['id'] = $db->insert_id("INSERT INTO " . MODULE_WALLET . "_transaction (
