...
class nukeviet_wallet
{
     ...
     * @return
     */
    public function getInfoPayment_site($data)
    {
        $return = array(
            'status' => 'ERROR',
            'message' => '',
        ...
        if (empty($order)) {
            ...
            $sql = "INSERT INTO " . NV_WALLET_TABLE . "_orders (
                ...
                :order_mod, :order_id, '', :order_object, :order_name, :money_amount, :money_unit, :secret_code, :url_back, :url_admin, " . NV_CURRENTTIME . ", 0, 4, 0
            )";
            try {
                $url_back = array();
     ...
     * @return
     */
    public function update($money, $money_unit, $userid,$customer_id, $message = "", $is_add = false, $dayamount = 0)
    {
        $this->is_error = false;
