...
function nv_wallet_money_in($userid, $money_unit, $money, $note_creat = '')
{
    ...
    $sql = "SELECT * FROM " . MODULE_WALLET . "_money WHERE userid=" . $userid . " AND money_unit=" . $db->quote($money_unit);
    $result = $db->query($sql);

    $return = false;

    if ($result->rowCount()) {
        $sql = "UPDATE " . MODULE_WALLET . "_money SET
            money_in= money_in+" . $money . ",
            money_total = money_total+" . $money . "
        WHERE userid= " . $userid . " AND money_unit=" . $db->quote($money_unit);
        $return = $db->exec($sql);
    } else {
        $sql = "INSERT INTO " . MODULE_WALLET . "_money (
...
function nv_wallet_exchange($userid, $money_unit, $moneyexchange, $moneysub, $totalmoneyexchangebefor)
{
    ...
    $check = $db->exec("UPDATE " . MODULE_WALLET . "_money SET
...
function getInfoMoney($money_unit)
{
    ...
    $sql = "SELECT * FROM " . MODULE_WALLET . "_money WHERE userid=" . $user_info['userid'] . " AND money_unit=" . $db->quote($money_unit);
    $result = $db->query($sql);
...
function nv_wallet_checkRate($money1, $money2)
{
    ...
    $sql = "SELECT exchange_from, exchange_to FROM " . MODULE_WALLET . "_exchange WHERE money_unit=" . $db->quote($money1) . " AND than_unit=" . $db->quote($money2);
    $result = $db->query($sql);
...
function updateTransaction($moneyexchange, $moneyunit, $status, $userid, $transinfo)
{
    ...
    $sql = "INSERT INTO " . MODULE_WALLET . "_transaction (
