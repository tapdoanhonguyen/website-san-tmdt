...
$sql = "SELECT * FROM " .$db_config['dbsystem']. '.'.  $db_config['prefix'] . "_" . $module_data . "_admin_groups ORDER BY group_title ASC";
...
if (!function_exists('nv_wallet_array_admins')) {
    ...
    function nv_wallet_array_admins($module_data)
    {
        ...
        $array = array();
        $sql = 'SELECT * FROM ' .$db_config['dbsystem']. '.'.  $db_config['prefix'] . '_' . $module_data . '_admins ORDER BY admin_id ASC';
...
if (!empty($module_info['admins'])) {
    ...
    foreach ($module_admin as $userid_i) {
        if (!isset($global_array_admins[$userid_i])) {
            $db->query('INSERT INTO ' .$db_config['dbsystem']. '.'.  $db_config['prefix'] . '_' . $module_data . '_admins (
...
if ($IS_FULL_ADMIN or !empty($PERMISSION_ADMIN['is_vtransaction']) or !empty($PERMISSION_ADMIN['is_mtransaction'])) {
	...
	$allow_func[] = 'transfers';
    $allow_func[] = 'withdrawal';
	$allow_func[] = 'viewtransaction';
	$submenu['transaction'] = $lang_module['transaction'];
	$submenu['transfers'] = $lang_module['transfers'];
	$submenu['withdrawal'] = $lang_module['withdrawal'];
...
if ($IS_FULL_ADMIN or !empty($PERMISSION_ADMIN['is_configmod'])) {
    $allow_func[] = 'config'; $allow_func[] = 'bank';$allow_func[] = 'bank_acount';
    $submenu['config'] = $lang_module['config_module'];  
	$submenu['bank'] = $lang_module['bank'];
	$submenu['bank_acount'] = $lang_module['bank_acount'];
}
