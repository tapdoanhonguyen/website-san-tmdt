 ...
 * @return
 */
function update_money($userid, $userid_receiver, $money, $money_unit = 'VND', $currTranStatus, $oldTranStatus, $status)
{
    ...
    $money_total_info = $sth->fetch();
				...
				$stmt = $db->prepare('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_money SET
            money_in=:money_in_receiver, money_total=:money_total_receiver
        WHERE userid=' . $userid_receiver . ' AND money_unit=' . $db->quote($money_unit));
		
				$stmt->bindValue(':money_in_receiver', ($money_total_info['money_in'] + $money), PDO::PARAM_INT);
                $stmt->bindValue(':money_total_receiver', ($money_total_info['money_total'] + $money), PDO::PARAM_INT);
            } 
			elseif ($currTranStatus != 4 and $oldTranStatus == 4) {
                // Cộng vào nếu trước đó đã trừ
                $stmt->bindValue(':money_in', ($row['money_in'] + $money), PDO::PARAM_INT);
