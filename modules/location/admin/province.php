...
// change status check
if ($nv_Request->isset_request('change_status_check', 'post, get')) {
    $provinceid = $nv_Request->get_int('provinceid', 'post, get', 0);
	$type = $nv_Request->get_int('type', 'post, get', 0);
    $content = 'NO_' . $provinceid;
    if($type==1){
		$query = 'SELECT ahamove FROM ' . $db_config['prefix'] . '_' . $module_data . '_province WHERE provinceid=' . $provinceid;
		$row = $db->query($query)->fetch();
	}else if($type==2){
		$query = 'SELECT ghtk FROM ' . $db_config['prefix'] . '_' . $module_data . '_province WHERE provinceid=' . $provinceid;
		$row = $db->query($query)->fetch();
	}else if($type==3){
		$query = 'SELECT ghn FROM ' . $db_config['prefix'] . '_' . $module_data . '_province WHERE provinceid=' . $provinceid;
		$row = $db->query($query)->fetch();
	}else if($type==4){
		$query = 'SELECT viettelpost FROM ' . $db_config['prefix'] . '_' . $module_data . '_province WHERE provinceid=' . $provinceid;
		$row = $db->query($query)->fetch();
	}else if($type==5){
		$query = 'SELECT vnpost FROM ' . $db_config['prefix'] . '_' . $module_data . '_province WHERE provinceid=' . $provinceid;
		$row = $db->query($query)->fetch();
	}
    if (isset($row['ahamove'])) {
        $ahamove = ($row['ahamove']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_province SET ahamove=' . intval($ahamove) . ' WHERE provinceid=' . $provinceid;
        $db->query($query);
        $content = 'OK_' . $provinceid;
    }else if (isset($row['ghtk'])) {
        $ghtk = ($row['ghtk']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_province SET ghtk=' . intval($ghtk) . ' WHERE provinceid=' . $provinceid;
        $db->query($query);
        $content = 'OK_' . $provinceid;
    }else if (isset($row['viettelpost'])) {
        $viettelpost = ($row['viettelpost']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_province SET viettelpost=' . intval($viettelpost) . ' WHERE provinceid=' . $provinceid;
        $db->query($query);
        $content = 'OK_' . $provinceid;
    }else if (isset($row['ghn'])) {
        $ghn = ($row['ghn']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_province SET ghn=' . intval($ghn) . ' WHERE provinceid=' . $provinceid;
        $db->query($query);
        $content = 'OK_' . $provinceid;
    }else if (isset($row['vnpost'])) {
        $vnpost = ($row['vnpost']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_province SET vnpost=' . intval($vnpost) . ' WHERE provinceid=' . $provinceid;
        $db->query($query);
        $content = 'OK_' . $provinceid;
    }
...
if ($nv_Request->isset_request('submit', 'post')) {
    $row['countryid'] = $nv_Request->get_int('countryid', 'post,get', 0);
    $row['vnpostid'] = $nv_Request->get_int('vnpostid', 'post,get', 0);
    $row['vtpid'] = $nv_Request->get_int('vtpid', 'post,get', 0);
    $row['ghnid'] = $nv_Request->get_int('ghnid', 'post,get', 0);
    ...
    if (empty($error)) {
        try {
            if (empty($row['provinceid'])) {
                $stmt = $db->prepare('INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_province (code, countryid,vnpostid,vtpid,ghnid, title, alias, type, weight) VALUES (:code, :countryid,:vnpostid,:vtpid,:ghnid, :title, :alias, :type, :weight)');
                
                $weight = $db->query('SELECT max(weight) FROM ' . $db_config['prefix'] . '_' . $module_data . '_province WHERE countryid=' . $row['countryid'])->fetchColumn();
                $weight = intval($weight) + 1;
                $stmt->bindParam(':weight', $weight, PDO::PARAM_INT);
            } else {
                $stmt = $db->prepare('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_province SET countryid = :countryid,vnpostid = :vnpostid,vtpid = :vtpid,ghnid = :ghnid, code = :code, title = :title, alias = :alias, type = :type WHERE provinceid=' . $row['provinceid']);
            }
            $stmt->bindParam(':code', $row['code'], PDO::PARAM_STR);
            $stmt->bindParam(':countryid', $row['countryid'], PDO::PARAM_STR);
            $stmt->bindParam(':vnpostid', $row['vnpostid'], PDO::PARAM_STR);
            $stmt->bindParam(':vtpid', $row['vtpid'], PDO::PARAM_STR);
            $stmt->bindParam(':ghnid', $row['ghnid'], PDO::PARAM_STR);
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
		...
		$xtpl->assign('CHECK_AHAMOVE', $view['ahamove'] == 1 ? 'checked' : '');
		$xtpl->assign('CHECK_GHTK', $view['ghtk'] == 1 ? 'checked' : '');
		$xtpl->assign('CHECK_GHN', $view['ghn'] == 1 ? 'checked' : '');
		$xtpl->assign('CHECK_VIETTELPOST', $view['viettelpost'] == 1 ? 'checked' : '');
		$xtpl->assign('CHECK_VNPOST', $view['vnpost'] == 1 ? 'checked' : '');
