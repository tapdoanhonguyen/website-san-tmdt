...
if ($nv_Request->isset_request('change_status_check', 'post, get')) {
    ...
    if($type==1){
		$query = 'SELECT ahamove FROM ' . $db_config['prefix'] . '_' . $module_data . '_ward WHERE wardid=' . $wardid;
		$row = $db->query($query)->fetch();
	}else if($type==2){
		$query = 'SELECT ghtk FROM ' . $db_config['prefix'] . '_' . $module_data . '_ward WHERE wardid=' . $wardid;
		$row = $db->query($query)->fetch();
	}else if($type==3){
		$query = 'SELECT ghn FROM ' . $db_config['prefix'] . '_' . $module_data . '_ward WHERE wardid=' . $wardid;
		$row = $db->query($query)->fetch();
	}else if($type==4){
		$query = 'SELECT viettelpost FROM ' . $db_config['prefix'] . '_' . $module_data . '_ward WHERE wardid=' . $wardid;
		$row = $db->query($query)->fetch();
	}else if($type==5){
		$query = 'SELECT vnpost FROM ' . $db_config['prefix'] . '_' . $module_data . '_ward WHERE wardid=' . $wardid;
		$row = $db->query($query)->fetch();
	}
    if (isset($row['ahamove'])) {
        $ahamove = ($row['ahamove']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_ward SET ahamove=' . intval($ahamove) . ' WHERE wardid=' . $wardid;
        $db->query($query);
        $content = 'OK_' . $wardid;
    }else if (isset($row['ghtk'])) {
        $ghtk = ($row['ghtk']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_ward SET ghtk=' . intval($ghtk) . ' WHERE wardid=' . $wardid;
        $db->query($query);
        $content = 'OK_' . $wardid;
    }else if (isset($row['viettelpost'])) {
        $viettelpost = ($row['viettelpost']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_ward SET viettelpost=' . intval($viettelpost) . ' WHERE wardid=' . $wardid;
        $db->query($query);
        $content = 'OK_' . $wardid;
    }else if (isset($row['ghn'])) {
        $ghn = ($row['ghn']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_ward SET ghn=' . intval($ghn) . ' WHERE wardid=' . $wardid;
        $db->query($query);
        $content = 'OK_' . $wardid;
    }else if (isset($row['vnpost'])) {
        $vnpost = ($row['vnpost']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_ward SET vnpost=' . intval($vnpost) . ' WHERE wardid=' . $wardid;
        $db->query($query);
        $content = 'OK_' . $wardid;
    }
    $nv_Cache->delMod($module_name);
    ...
    if (empty($error)) {
        try {
            if (empty($row['wardid'])) {
                ...
                $stmt = $db->prepare('INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_ward ( districtid,vnpostid,vtpid,ghnid, title, alias, type, location) VALUES ( :districtid,:vnpostid,:vtpid,:ghnid, :title, :alias, :type, :location)');
            } else {
                $stmt = $db->prepare('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_ward SET  districtid = :districtid,vnpostid = :vnpostid,vtpid = :vtpid,ghnid = :ghnid, title = :title, alias = :alias, type = :type, location = :location WHERE wardid=' . $row['wardid']);
            }
