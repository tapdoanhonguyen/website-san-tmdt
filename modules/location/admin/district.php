...
if ($nv_Request->isset_request('change_status_check', 'post, get')) {
    ...
    if($type==1){
		$query = 'SELECT ahamove FROM ' . $db_config['prefix'] . '_' . $module_data . '_district WHERE districtid=' . $districtid;
		$row = $db->query($query)->fetch();
	}else if($type==2){
		$query = 'SELECT ghtk FROM ' . $db_config['prefix'] . '_' . $module_data . '_district WHERE districtid=' . $districtid;
		$row = $db->query($query)->fetch();
	}else if($type==3){
		$query = 'SELECT ghn FROM ' . $db_config['prefix'] . '_' . $module_data . '_district WHERE districtid=' . $districtid;
		$row = $db->query($query)->fetch();
	}else if($type==4){
		$query = 'SELECT viettelpost FROM ' . $db_config['prefix'] . '_' . $module_data . '_district WHERE districtid=' . $districtid;
		$row = $db->query($query)->fetch();
	}else if($type==5){
		$query = 'SELECT vnpost FROM ' . $db_config['prefix'] . '_' . $module_data . '_district WHERE districtid=' . $districtid;
		$row = $db->query($query)->fetch();
	}
	
   if($type==1) {
        $ahamove = ($row['ahamove']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_district SET ahamove=' . intval($ahamove) . ' WHERE districtid=' . $districtid;
        $db->query($query);
        $content = 'OK_' . $districtid;
    }else if ($type==2) {
        $ghtk = ($row['ghtk']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_district SET ghtk=' . intval($ghtk) . ' WHERE districtid=' . $districtid;
        $db->query($query);
        $content = 'OK_' . $districtid;
    }else if ($type==4) {
        $viettelpost = ($row['viettelpost']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_district SET viettelpost=' . intval($viettelpost) . ' WHERE districtid=' . $districtid;
        $db->query($query);
        $content = 'OK_' . $districtid;
    }else if ($type==3) {
        $ghn = ($row['ghn']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_district SET ghn=' . intval($ghn) . ' WHERE districtid=' . $districtid;
        $db->query($query);
        $content = 'OK_' . $districtid;
    }else if ($type==5) {
        $vnpost = ($row['vnpost']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_district SET vnpost=' . intval($vnpost) . ' WHERE districtid=' . $districtid;
        $db->query($query);
        $content = 'OK_' . $districtid;
    }
    $nv_Cache->delMod($module_name);
    ...
    if (empty($error)) {
        try {
            if (empty($row['districtid'])) {
                $stmt = $db->prepare('INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_district (code, provinceid,vnpostid,vtpid,ghnid, title, alias, type, location, weight) VALUES (:code, :provinceid,:vnpostid,:vtpid,:ghnid, :title, :alias, :type, :location, :weight)');
                
                $weight = $db->query('SELECT max(weight) FROM ' . $db_config['prefix'] . '_' . $module_data . '_district')->fetchColumn();
                $weight = intval($weight) + 1;
                $stmt->bindParam(':weight', $weight, PDO::PARAM_INT);
            } else {
                $stmt = $db->prepare('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_district SET code = :code, provinceid = :provinceid,vnpostid = :vnpostid,vtpid = :vtpid,ghnid = :ghnid, title = :title, alias = :alias, type = :type, location = :location WHERE districtid=' . $row['districtid']);
            }
