...
if ($nv_Request->isset_request('submit', 'post')) {
	...
	$list_quanhuyen=get_tinhthanh_vnpost();
        ...
        try {
           foreach($list_quanhuyen as $location){
			$row['type'] = 'Tỉnh';
			 ...
			 $alias = change_alias($location['TenTinhThanh']);
				$alias = strtolower($alias);

				$stmt = $db->prepare('SELECT COUNT(*) FROM ' . $db_config['prefix'] . '_' . $module_data . '_province WHERE alias = :alias');
				$stmt->bindParam(':alias', $alias, PDO::PARAM_STR);
				$stmt->execute();

				if ($stmt->fetchColumn()) {
					$weight = $db->query('SELECT MAX(districtid) FROM ' . $db_config['prefix'] . '_' . $module_data . '_province')->fetchColumn();
					$weight = intval($weight) + 1;
					$alias = $alias . '-' . $weight;
				}
				$row['alias']=$alias;
			
			$stmt = $db->prepare('INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_province (code, provinceid,vnpostid, title, alias, type, countryid,  weight) VALUES (:code, :provinceid,:vnpostid,:title, :alias, :type,:countryid,  :weight)');
            ...
            $stmt->bindParam(':provinceid', $location['MaTinhThanh'], PDO::PARAM_STR);
			$stmt->bindParam(':vnpostid', $location['MaTinhThanh'], PDO::PARAM_STR);
            $stmt->bindParam(':title', $location['TenTinhThanh'], PDO::PARAM_STR);
    ...
    $row = $db->query('SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_district WHERE provinceid=' . $row['provinceid'])->fetch();
    if (empty($row)) {
