...
if (! defined('NV_MAINFILE'))
    die('Stop!!!');
global $nv_Cache, $db, $module_name, $module_data, $global_config, $db_config, $nv_Request;
$location_array_config = array();
$_sql = 'SELECT config_name, config_value FROM ' . $db_config['prefix'] . '_config';
...
if ($nv_Request->isset_request('location_reload', 'post,get')) {
	...
	$location_html = nv_location_build_sea($data_config);
    die($location_html);
}
 ...
 *
 */
function nv_location_build_sea($_data_config = array(), $template = 'default', $module = 'location')
{
    global $db, $db_config, $site_mods, $global_config, $lang_module, $location_array_config;
    ...
    $xtpl = new XTemplate('form_input_sea.tpl', NV_ROOTDIR . '/themes/' . $template . '/modules/' . $site_mods[$module]['module_file']);
    $xtpl->assign('LANG', $lang_module);
    $xtpl->assign('CONFIG', $data_config);
