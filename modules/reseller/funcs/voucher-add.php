...
if ($mod == 'get_list_product') {
	...
	$db->sqlreset()
		->select('COUNT(*)')
		->from('' . TABLE . '_product')
		->where(' status = 1 AND inhome = 1 AND store_id = ' . $store_id . $where);
...
if ($mod == 'add_voucher') {
	...
	// 	$row['time_to'] = 0;
	// }
	$row['time_from'] = strtotime($nv_Request->get_string('time_from', 'post, get', ''));
    $row['time_to'] = strtotime($nv_Request->get_string('time_to', 'post, get', ''));
	...
	if ($row['time_from'] > $row['time_to']) {
		$json[] = ['status' => 'ERROR_date', 'mess' => 'Ngày từ không được lớn hơn ngày đến','name' => $row['time_from']];
		print_r(json_encode($json[0]));
		die();
	}
...
$contents = nv_theme_voucher_add($data);
