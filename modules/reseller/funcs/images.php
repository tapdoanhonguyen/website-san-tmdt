...
if ($nv_Request->isset_request('delete_other_images_tmp', 'post')) {
    $homeimgfile = $nv_Request->get_title('homeimgfile', 'post', '');
    $thumb = $nv_Request->get_title('thumb', 'post', '');
   
	if(!empty($homeimgfile))
		$homeimgfile = '/data/tmp/' . $homeimgfile;
    
    if (! nv_delete_other_images_tmp(NV_ROOTDIR . '' . $homeimgfile, NV_ROOTDIR . '' . $thumb)){
        die('ERROR');
    }else{
        die('OK'); 
    }
...
$place_info = $db->query('SELECT * FROM ' . TMS_BDS . ' WHERE id=' . $rows_id)->fetch();

$result = $db->query('SELECT * FROM ' . TMS_BDS . '_images WHERE rows_id=' . $rows_id);
    ...
    if (empty($error)) {
            ...
            if ($otherimage['id'] == 0) {
                if (file_exists(NV_ROOTDIR . '/' . NV_TEMP_DIR . '/' . $otherimage['homeimgfile'])) {
                    ...
                    if (@nv_copyfile(NV_ROOTDIR . '/' . NV_TEMP_DIR . '/' . $otherimage['homeimgfile'], NV_ROOTDIR . '/' . $currentpath . '/' . $otherimage['homeimgfile'])) {
                        ...
                        $sth = $db->prepare('INSERT INTO ' . TMS_BDS . '_images( rows_id,idsite, title, description, homeimgfile, weight ) VALUES ( :rows_id,'.intval( $global_config['idsite'] ).', :title, :description, :homeimgfile, 0)');
        ...
        foreach ($array_id_old as $id_old) {
            if (! in_array($id_old, $array_id_new)) {
                $rows = $db->query('SELECT * FROM ' . TMS_BDS . '_images WHERE id=' . $id_old)->fetch();
                if (! empty($rows)) {
                    $db->query('DELETE FROM ' . TMS_BDS . '_images WHERE id = ' . $id_old);
