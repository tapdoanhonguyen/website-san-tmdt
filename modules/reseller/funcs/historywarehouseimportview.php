...
$array_mod_title[] = array(
	...
	'link' => NV_MY_DOMAIN .'/'. $module_name .'/historywarehouseimport/'
);
$array_mod_title[] = array(
	'catid' => 0,
	'title' => $page_title,
	'link' => nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=historywarehouseimportview&amp;warehouse_product_import_id=' . $row['warehouse_product_import_id'].'&warehouse_product_import_code='.$row['warehouse_product_import_code'].'&time_add='.$row['time_add'].'&user_add='.$row['user_add'].'&name_warehouse='.$row['name_warehouse'].'&total_price='.$row['total_price'].'&total_amount='.$row['total_amount'],true)
);
