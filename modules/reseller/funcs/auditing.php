<?php
	...
	*/
	...
	$where = '';
	$base_url = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op .'&sea_flast='.  $sea_flast;
	...
	if($mod=='is_download'){
		...
		foreach ($list_order as $view) {
			...
			// phí vnpay
			$data_array['vnpay'] = phi_vnpay_order($view, $status);
			...
			// phí phạt
			$temp = phi_phat_order($view, $status);
			$data_array['phi_phat'] = $temp ? $temp : 0;
	...
	if ($nv_Request->isset_request('complain', 'post,get'))
	{
		// kiểm tra thông tin khiếu nại đã tồn tại chưa.
		$check = $db->query('SELECT id FROM '. TABLE .'_order_punish_complain WHERE store_id ='. $store_id .' AND time_from ='. $ngay_tu .' AND time_to ='. $ngay_den)->fetchColumn();
		
		if(!$check)
		{
			$row['time_add'] = NV_CURRENTTIME;
			
			$stmt = $db->prepare('INSERT INTO ' . TABLE . '_order_punish_complain (store_id, time_from, time_to, time_add) VALUES (:store_id, :time_from, :time_to, :time_add)');
			
            $stmt->bindParam(':time_add', $row['time_add'], PDO::PARAM_INT);
			$stmt->bindParam(':store_id', $store_id, PDO::PARAM_INT);
            $stmt->bindParam(':time_from', $tuannay['from'], PDO::PARAM_INT);
            $stmt->bindParam(':time_to', $tuannay['to'], PDO::PARAM_INT);
	...
	// Tên shop
	$view['company_name'] = $global_seller['company_name'];
	...
	if($ngay_den > 0){
		...
		$check = $db->query('SELECT id FROM '. TABLE .'_order_punish_complain WHERE store_id ='. $store_id .' AND time_from ='. $ngay_tu .' AND time_to ='. NV_CURRENTTIME)->fetchColumn();
	}
	
	if(!$check)
	{
		$xtpl->parse('main.complain');
	}
