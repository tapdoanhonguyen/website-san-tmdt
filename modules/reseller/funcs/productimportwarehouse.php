...
if ($nv_Request->isset_request('submit', 'post')) {
	 ...
	 $row['warehouse_id'] = get_info_warehouse_store($store_id)['id'];
			 ...
			 if ($exc) {
                ...
                if (empty($row['warehouse_product_import_id'])) {
					$row['warehouse_product_import_id']=$db->query('SELECT max(warehouse_product_import_id) FROM ' . TABLE. '_warehouse_import')->fetchColumn();
					 ...
					 nv_insert_logs(NV_LANG_DATA, $module_name, 'Add warehouse_import', ' ', $user_info['userid']);
				}else{
					  nv_insert_logs(NV_LANG_DATA, $module_name, 'Edit warehouse_import', 'ID: ' . $row['warehouse_product_import_id'], $user_info['userid']);
				}
			 }
...
if(!empty($list_classify)){
	...
	foreach($list_unit_currency as $value){
		$xtpl->assign('unit_currency', array(
			'key' => $value['id'],
			'title' => $value['name_currency'].' ('.$value['symbol'].')',
			'selected' => ($value['id'] == $value_list['unit_currency']) ? ' selected="selected"' : ''));
...
$array_mod_title[] = array(
		'catid' => 0,
		'title' => $lang_module['product_import_warehouse'],
