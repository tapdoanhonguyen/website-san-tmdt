...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        ...
        $view['link_view'] = nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=historywarehouseimportview&amp;warehouse_product_import_id=' . $view['warehouse_product_import_id'].'&warehouse_product_import_code='.$view['warehouse_product_import_code'].'&time_add='.$view['time_add'].'&user_add='.$view['user_add'].'&name_warehouse='.$view['name_warehouse'].'&total_price='.$view['total_price'].'&total_amount='.$view['total_amount'],true);
		$view['total_price']=number_format($view['total_price']).' VND';
		$view['total_amount']=number_format($view['total_amount']);
        $view['link_delete'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;delete_warehouse_product_import_id=' . $view['warehouse_product_import_id'] . '&amp;delete_checkss=' . md5($view['warehouse_product_import_id'] . NV_CACHE_PREFIX . $client_info['session_id']);
        $xtpl->assign('VIEW', $view);
        $xtpl->parse('main.view.loop');
    }
	$list_warehouse = get_warehouse_store($store_id );
	foreach($list_warehouse as $value){
		...
		$xtpl->parse('main.view.warehouse_id');
	}
