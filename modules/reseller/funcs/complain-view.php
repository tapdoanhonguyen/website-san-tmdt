<?php
	...
	*/
	...
	if(!$row['order_id'])
	{
		nv_redirect_location(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=complain');
	}
	
	
	// check thông tin order_id đã khiếu nại chưa.
	$row = $db->query('SELECT t1.* FROM '. TABLE .'_complain t1, ' . TABLE . '_order t2 WHERE t1.order_id = t2.id AND t2.store_id = "'. $store_id .'" AND t1.order_id ='.$row['order_id'])->fetch();
	...
	if($row['images_video'])
	{
		$row['images_video'] = $_SERVER["chonhagiau"] . NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $row['images_video'];	
	}
	...
	if($list_image)
	{
		for($i = 0; $i <= 7; $i++)
		{
			...
			if (!empty($list_image[$i])) {
				
				$src_image = $_SERVER["chonhagiau"] . NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $list_image[$i];			
