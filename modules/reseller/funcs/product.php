	...
	*/
	...
	if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
		...
		if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
			...
			if(check_product($id))
			{
				$db->query('UPDATE ' . TABLE . '_rows SET status = 0, inhome = 0 WHERE idsite ='. $store_id .' AND id=' . intval($id));
	...
	$where1 = '';
	$status = $nv_Request->get_int( 'status', 'post,get',$_SESSION['status']);
	$q = $nv_Request->get_title( 'q', 'post,get',$_SESSION['q'] );
	...
	$page = $nv_Request->get_int('page', 'post,get',$_SESSION['page']);
	...
	$check_status = variableChanged($status);
	$check_SessionStatus = variableChanged($_SESSION['status']);
	
	if(!$check_SessionStatus){
		$_SESSION['page'] = $page;
	}
	elseif($check_status != $check_SessionStatus){
		$page = 1;
		$_SESSION['page'] = 1;
	}
	elseif(($q != '' || $q == '') && ($check_status != $check_SessionStatus)){
		$page = 1;
	}
	elseif($check_status == $check_SessionStatus){
		$_SESSION['page'] = $page;
	...
	if( $mod == 'tab_product_status' )
	{
		...
		elseif($status == 3 ){
			...
			$where .= ' AND t1.status = 1 AND product_number <= 0';
			$where1 .= ' AND t2.status = 1 AND inhome = 0 AND t1.product_number = 0';
			$base_url .= '&status=2';
		}
		elseif($status == 4){
			
			$where .= ' AND t1.status = 3 ';
			$where1 .= ' AND t2.status = 3 ';
			$base_url .= '&status=3';
		}
		$per_page = 20;
		$db->sqlreset()
		->select('COUNT(t2.id)')
		->from('' . TABLE .'_rows t2')
		->where('t2.idsite = ' . $store_id . $where1);
		$sth = $db->prepare($db->sql());		
		$sth->execute();
		$num_items = $sth->fetchColumn();
		//$data=$db->query('SELECT * from '. TABLE .'_rows order by addtime DESC limit '.$per_page. ' offset '.($page - 1) * $per_page);
		...
		if($status == 2 || $status == 4){
			$xtpl->assign('d_none','d-none');
			$xtpl->assign('show_name','d-block');
			$xtpl->assign('text_align','text-right');
		}
		if($status == 4){
			$xtpl->assign('show_bug','d-block');
		}
		
		$data = $db->query('SELECT t1.* FROM '. TABLE . '_rows t1 WHERE t1.idsite = '. $store_id . $where . ' order by t1.addtime DESC limit '.$per_page. ' offset '.($page - 1) * $per_page)->fetchAll();
		if(!empty($data))
		{
			foreach ( $data as $view ) {
				...
				$view['alias'] =  nv_url_rewrite( NV_BASE_SITEURL .  $view['alias'] . '-' . $view['id'] . '/',true);
				$view['categories_id'] = $global_array_shops_cat[$view['listcatid']]['title'];
				// Xac dinh anh nho
				if ($view['homeimgthumb'] == 1) {
					//image thumb

					$thumb = NV_BASE_SITEURL . NV_FILES_DIR . '/' . $store['username'] . '/shops/' . $view['homeimgfile'];
					$imghome = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $store['username']  . '/shops/' . $view['homeimgfile'];
				} elseif ($view['homeimgthumb'] == 2) {
					//image file

					$imghome = $thumb = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $store['username']  . '/shops/' . $view['homeimgfile'];
				} elseif ($view['homeimgthumb'] == 3) {
					//image url

					$imghome = $thumb = $view['homeimgfile'];
				} elseif (file_exists(NV_ROOTDIR . '/themes/' . $theme . '/images/shops/no-image.jpg')) {
					$imghome = $thumb = NV_STATIC_URL . 'themes/' . $theme . '/images/shops/no-image.jpg';
				} else {
					$imghome = $thumb = NV_STATIC_URL . 'themes/default/images/' . $module_file . '/no-image.jpg';
				}
				$view['homeimgfile'] = $imghome;
				if (!empty($view['image']) and is_file(NV_UPLOADS_REAL_DIR . '/' . $store['username']  . '/' . $module_upload . '/' . $view['image'])) {
					$view['image'] = NV_BASE_SITEURL . NV_FILES_DIR . '/' . $store['username']  . '/' . $module_upload. '/' . $view['image'];
					}else{
					$domain=explode('.', $_SERVER["SERVER_NAME"]);
					$server = $domain[1].'.'.$domain[2];
					$view['image'] ='https://'. $server .NV_BASE_SITEURL . NV_FILES_DIR . '/' . $store['username']  . '/' . $module_upload. '/' . $view['image'];
				}
				
				$xtpl->assign( 'CHECK_FREESHIP', $view['free_ship'] == 1 ? 'checked' : '' );
				$xtpl->assign( 'disabled_freeship', $view['free_ship'] == 0 ? 'disabled=disabled' : '' );
				$xtpl->assign( 'disabled_fs', $view['free_ship'] > 0 ? '' : 'disabled' );
				...
				$view['warehouse'] = $db->query('SELECT sum(quantity) as sum FROM ' . TABLE .'_group_quantity WHERE pro_id ='. $view['id'])->fetchColumn();
				...
				$list_classify = $db->query('SELECT * FROM ' . TABLE .'_group_items WHERE pro_id ='. $view['id'])->fetchAll();
				...
				if($list_classify)
				{
					...
					foreach($list_classify as $classify)
					{
						...
						$list_classify_value = $db->query('SELECT * FROM ' . TABLE .'_group WHERE groupid ='. $classify['group_id'])->fetchAll();	
					...
					if(!empty($where))
					{
						$list_product_price = $db->query('SELECT * FROM ' . TABLE .'_group_quantity WHERE ' . $where)->fetchAll();
					}
	...
	if($mod == 'edit_product')
	{
		...
		foreach (array_combine($classify_id_arr, $product_price_arr) as $id_product => $price_product) {
			...
			$db->query($db->query('UPDATE '. TABLE . '_rows t1 INNER JOIN ' . TABLE .'_product_classify_value_product t2 ON t1.id = t2.product_id SET t2.price = ' . $price_product . ' , t1.price = ' . $price_product . ' WHERE t2.id = ' . $id_product. ' AND t1.idsite = '.$store_id));
		}
		...
		foreach (array_combine($classify_id_arr, $price_special_product_arr) as $id_product => $price_special_product) {
			...
			$db->query($db->query('UPDATE '. TABLE . '_rows t1 INNER JOIN ' . TABLE .'_product_classify_value_product t2 ON t1.id = t2.product_id SET t2.price_special = ' . $price_special_product . ' , t1.price_special = ' . $price_special_product . ' WHERE t2.id = ' . $id_product. ' AND t1.idsite = '.$store_id));
		}
		foreach (array_combine($classify_id_arr, $quantily_product_arr) as $id_product => $quantily_product) {
			...
			$db->query($db->query('UPDATE '. TABLE . '_rows t1 INNER JOIN ' . TABLE .'_product_classify_value_product t2 ON t1.id = t2.product_id SET t2.sl_tonkho = ' . $quantily_product . ' WHERE t2.id = ' . $id_product. ' AND t1.idsite = '.$store_id));
		}
	...
	if($mod == 'edit_product1')
	{
		...
		$db->query($db->query('UPDATE '. TABLE . '_rows SET price_special = ' . $price_special_product1 . ' , price = '. $price_product1 .' WHERE id = ' . $product_id1. ' AND idsite = '.$store_id));
		
		$db->query($db->query('UPDATE '. TABLE . '_rows t1 INNER JOIN ' . TABLE .'_product_classify_value_product t2 ON t1.id = t2.product_id SET t2.sl_tonkho = ' . $quantily_product1 . ' , t2.price = ' . $price_product1 . ' , t2.price_special = ' . $price_special_product1 . ' WHERE t1.id = ' . $product_id1. ' AND t1.idsite = '.$store_id));
	...
	if($mod=='is_download'){
		...
		set_time_limit( 0 );
		$list_product = $db->query('SELECT * FROM ' . TABLE .'_rows WHERE status = 1 '. $where . ' ORDER BY store_id')->fetchAll();
		$stt = 0;
		foreach ($list_product as $view) {
			...
			// danh mục
			$current_category = $db->query('SELECT parrent_id FROM '. TABLE .'_catalogs WHERE id = '. $view['categories_id'])->fetchColumn();
	...
	if ($nv_Request->isset_request('change_freeship', 'post, get')) {
		...
		$query = 'SELECT free_ship FROM ' . TABLE . '_rows WHERE userid = ' . $store_id .' AND id=' . $id;
		$row = $db->query($query)->fetch();
		
		
		if (isset($row['free_ship']) and $row['free_ship'] >= 0){
			$free_ship = ($row['free_ship']) ? 0 : 1;
			$query = 'UPDATE ' . TABLE . '_rows SET free_ship=' . intval($free_ship) . ' WHERE id=' . $id;
			$db->query($query);
			// kiểm tra product_id, _retails_product_detail
			if($free_ship)
			{
				$data_free_ship[] = array(
					'area' => "0",
					'province' => array("0" => "0")
					);
				
				$json_free_ship = json_encode($data_free_ship);
				$check_retails_product_detail = $db->query('SELECT product_id FROM ' . TABLE . '_product_detail WHERE product_id=' . $id)->fetchColumn();
				
				if($check_retails_product_detail)
				{
					$db->query('UPDATE ' . TABLE . '_product_detail SET json_free_ship='.$db->quote($json_free_ship).' WHERE product_id ='. $id);
				}
				else
				{
					$sql = 'INSERT INTO ' . TABLE . '_product_detail (product_id,json_free_ship) VALUES (:product_id,:json_free_ship)';
							$data_insert = array();
							$data_insert['product_id'] = $id;
							$data_insert['json_free_ship'] = $json_free_ship;
							$db->insert_id($sql, 'product_id', $data_insert);
				}
			}
			else
			{
				$db->query('DELETE FROM ' . TABLE . '_product_detail WHERE product_id=' . $id);
			}
			$content = 'OK_' . $id;
		}
		$nv_Cache->delMod($module_name);
	...
	$contents = nv_theme_shops_list_product($per_page,$page,$num_items,$base_url,$ngay_tu,$ngay_den,$status_ft,$sea_flast,$show_view,$q,$catalogy,$status);
