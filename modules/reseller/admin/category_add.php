...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $row['origin'] = $nv_Request->get_array('origin', 'post', '');
    $row['origin_list'] = '';
    $jj = 1;

    foreach ($row['origin'] as $key => $value) {
        if($jj<count($row['origin'])){
            $row['origin_list'] .= $value . '|';
        }else{
            $row['origin_list'] .= $value;
        }
        $jj++;
    ...
    if (empty($error)) {
     ...
     try {
        if (empty($row['id'])) {
            ...
            $stmt = $db->prepare('INSERT INTO ' . TABLE . '_categories (other_image,origin,brand, name, alias, parrent_id, keyword, image, description, viewdescriptionhtml, groups_view, inhome, viewcat, numlinks, idsite, user_add, time_add,weight, status) VALUES (:other_image,:origin,:brand, :name, :alias, :parrent_id, :keyword, :image, :description, :viewdescriptionhtml, :groups_view, :inhome, :viewcat, :numlinks, :idsite, :user_add, :time_add, :weight, :status)');
            ...
            $stmt = $db->prepare('UPDATE ' . TABLE . '_categories SET other_image = :other_image ,origin = :origin ,brand = :brand, name = :name, alias = :alias, keyword = :keyword, image = :image, description = :description, viewdescriptionhtml = :viewdescriptionhtml, groups_view = :groups_view, user_edit='.$admin_info['userid'].',time_edit='.NV_CURRENTTIME.',parrent_id='.$row['parrent_id'].' WHERE id=' . $row['id']);
        }
        ...
        $stmt->bindParam(':origin', $row['origin_list'], PDO::PARAM_STR);
