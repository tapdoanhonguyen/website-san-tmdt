...
if (!nv_function_exists('nv_block_shops_groups')) {
    /**
     ...
     * @return string
     */
    function nv_block_config_shops_groups($module, $data_block, $lang_block)
    {
        ...
        $sql = 'SELECT * FROM ' . $db_config['prefix'] . '_' . $site_mods[$module]['module_data'] . '_block_cat ORDER BY weight ASC';
        ...
        foreach ($list as $l) {
            ...
            $html .= '<option value="' . $l['bid'] . '" ' . (($data_block['blockid'] == $l['bid']) ? ' selected="selected"' : '') . '>' . $l[NV_LANG_DATA . '_title'] . '</option>';
        }
        $html .= '</select>';
        $html .= $html_input;
     ...
     * @return array
     */
    function nv_block_config_shops_groups_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = [];
        $return['error'] = [];
     ...
     * @return string|void
     */
    function nv_block_shops_groups($block_config)
    {
        global $module_array_cat, $site_mods, $module_config, $global_config, $nv_Cache, $db, $db_config;
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        if ($module == $module_name) {
            ...
            $module_array_cat = [];
            $sql = 'SELECT catid, parentid, title, alias, viewcat, subcatid, numlinks, description, keywords, groups_view, status FROM ' . $db_config['prefix'] . '_' . $site_mods[$module]['module_data'] . '_cat ORDER BY sort ASC';
        ...
        $content = nv_block_shops_groups($block_config);
    }
}
