...
if (!nv_function_exists('nv_home_danhmuc')) {
	/**
		* @param string $module
		...
		* @return string
	*/
	function nv_config_home_danhmuc($module, $data_block, $lang_block)
	{
	...
	/**
		...
		* @return number
	*/
	function nv_config_home_danhmuc_submit($module, $lang_block)
	{
		global $nv_Request;
	...
	function get_categories_all_lev_block($id, $block_config)
	{
		...
		$categorys = $db->query('SELECT catid, ' . NV_LANG_DATA .'_title as name, ' . NV_LANG_DATA .'_alias as alias, image, parentid FROM ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $block_config['module'] .'_catalogs WHERE inhome = 1 AND parentid =' . $id .' ORDER BY weight ASC')->fetchAll();
		...
		foreach($categorys as $category)
		{
			...
			$category['sub'] = get_categories_all_lev_block($category['catid'], $block_config);
	...
	/**
		...
		* @return string
	*/
	function nv_home_danhmuc($block_config)
	{
		...
		$global_categories = get_categories_all_lev_block(0, $block_config);
...
if (defined('NV_SYSTEM')) {
	$content = nv_home_danhmuc($block_config);
}
