...
elseif( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	if( $nv_Request->get_int( 'save', 'post' ) == 1 )
	{
		...
		$data['type_link'] = $nv_Request->get_int( 'type_link', 'post', 0 );
		$data['alias_type_link'] = nv_substr( $nv_Request->get_title( 'alias_type_link', 'post', '', '' ), 0, 250 );
			...
			if( $data['photo_id'] == 0 )
			{
				try
				{
					...
					$stmt = $db_slave->prepare( 'INSERT INTO ' . TABLE_SLIDER_NAME . '_photo SET 
						...
						links =:links,
						type_link =:type_link,
						alias_type_link =:alias_type_link,
					...
					$stmt->bindParam( ':type_link', $data['type_link'], PDO::PARAM_INT );
					$stmt->bindParam( ':alias_type_link', $data['alias_type_link'], PDO::PARAM_STR );
	...
	foreach( $array_type_link as $key => $name )
	{
		$xtpl->assign( 'type_link', array(
			'key' => $key,
			'name' => $name,
			'selected' => ( $key == $data['type_link'] ) ? 'selected="selected"' : '' ) );
		$xtpl->parse( 'main.type_link' );
	}
