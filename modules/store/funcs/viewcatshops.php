...
if ($mod == "follow") {
	if (!defined('NV_IS_USER')) {
		...
		$alias_shop = $db->query("SELECT username FROM " . NV_TABLE_USER . " where userid = " . $info_shop['userid'])->fetchColumn();
		...
		$check = $db->query('SELECT count(1) FROM ' . TABLE . '_follow WHERE shop_id = ' . $id . ' AND user_id = ' . $user_info['userid'])->fetchColumn();
		if (!$check) {
...
if ($mod == "save_voucher") {
	...
	if (!defined('NV_IS_USER')) {
		...
		$check_voucher_shop = $db->query('SELECT COUNT(1) FROM ' . TABLE . '_voucher_shop WHERE store_id = ' . $_SESSION["shop_id"] . ' AND id = ' . $voucher_id)->fetchColumn();
		if ($check_voucher_shop) {
			$check_voucher_wallet = $db->query('SELECT COUNT(1) FROM ' . TABLE . '_voucher_wallet_shop WHERE userid = ' . $user_info["userid"] . ' AND voucherid = ' . $voucher_id)->fetchColumn();
		...
		$sql = 'INSERT INTO ' . TABLE . '_voucher_wallet_shop (voucherid, userid, time_add) VALUES (:voucherid, :userid, :time_add)';
		$data_insert = array();
...
if (true) {
	...
	if($user_info['userid']){
		$list_voucher = $db->query('SELECT * FROM ' . TABLE . '_voucher_shop t1 WHERE t1.store_id = ' . $shop_id . ' AND t1.status = 1 AND t1.time_to > ' . $today . ' AND NOT EXISTS (SELECT id FROM ' . TABLE . '_order_voucher_shop t2 WHERE t2.voucherid = t1.id and t2.status = 1 and t2.userid = ' . $user_info['userid'] . ') ORDER BY t1.time_to ASC')->fetchAll();
	}else{
		$list_voucher = $db->query('SELECT * FROM ' . TABLE . '_voucher_shop t1 WHERE t1.store_id = ' . $shop_id . ' AND t1.status = 1 AND t1.time_to > ' . $today . ' ORDER BY t1.time_to ASC')->fetchAll();
	}
