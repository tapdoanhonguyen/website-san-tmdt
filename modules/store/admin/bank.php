...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    $query = 'SELECT status FROM ' . TABLE . '_bank WHERE bank_id=' . $bank_id;
    $row = $db->query($query)->fetch();
    if (isset($row['status']))     {
        $status = ($row['status']) ? 0 : 1;
        $query = 'UPDATE ' . TABLE . '_bank SET status=' . intval($status) . ' WHERE bank_id=' . $bank_id;
        $db->query($query);
        $content = 'OK_' . $bank_id;
    }
...
if ($nv_Request->isset_request('ajax_action', 'post')) {
    ...
    $content = 'NO_' . $bank_id;
    if ($new_vid > 0)     {
        $sql = 'SELECT bank_id FROM ' . TABLE . '_bank WHERE bank_id!=' . $bank_id . ' ORDER BY weight ASC';
        $result = $db->query($sql);
        $weight = 0;
        while ($row = $result->fetch())
        {
            ++$weight;
            if ($weight == $new_vid) ++$weight;             $sql = 'UPDATE ' . TABLE . '_bank SET weight=' . $weight . ' WHERE bank_id=' . $row['bank_id'];
            $db->query($sql);
        }
        $sql = 'UPDATE ' . TABLE . '_bank SET weight=' . $new_vid . ' WHERE bank_id=' . $bank_id;
        $db->query($sql);
        $content = 'OK_' . $bank_id;
    }
    $nv_Cache->delMod($module_name);
...
if ($nv_Request->isset_request('delete_bank_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($bank_id > 0 and $delete_checkss == md5($bank_id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $weight=0;
        $sql = 'SELECT weight FROM ' . TABLE . '_bank WHERE bank_id =' . $db->quote($bank_id);
        $result = $db->query($sql);
        list($weight) = $result->fetch(3);
        
        $db->query('DELETE FROM ' . TABLE . '_bank  WHERE bank_id = ' . $db->quote($bank_id));
        if ($weight > 0)         {
            $sql = 'SELECT bank_id, weight FROM ' . TABLE . '_bank WHERE weight >' . $weight;
            $result = $db->query($sql);
            while (list($bank_id, $weight) = $result->fetch(3))
            {
                $weight--;
                $db->query('UPDATE ' . TABLE . '_bank SET weight=' . $weight . ' WHERE bank_id=' . intval($bank_id));
            }
        }
        $nv_Cache->delMod($module_name);
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($error)) {
        try {
            if (empty($row['bank_id'])) {
                ...
                $stmt = $db->prepare('INSERT INTO ' . TABLE . '_bank (bank_code, name_bank, user_add, time_add, status, weight) VALUES (:bank_code, :name_bank, :user_add, :time_add, :status, :weight)');

                $stmt->bindParam(':user_add', $row['user_add'], PDO::PARAM_INT);
                $stmt->bindParam(':time_add', $row['time_add'], PDO::PARAM_INT);
                $stmt->bindValue(':status', 1, PDO::PARAM_INT);

                $weight = $db->query('SELECT max(weight) FROM ' . TABLE . '_bank')->fetchColumn();
                $weight = intval($weight) + 1;
                $stmt->bindParam(':weight', $weight, PDO::PARAM_INT);


            } else {
                $stmt = $db->prepare('UPDATE ' . TABLE . '_bank SET bank_code = :bank_code, name_bank = :name_bank,user_edit='.$admin_info['userid'].',time_edit='.NV_CURRENTTIME.' WHERE bank_id=' . $row['bank_id']);
            }
    ...
    $row = $db->query('SELECT * FROM ' . TABLE . '_bank WHERE bank_id=' . $row['bank_id'])->fetch();
    if (empty($row)) {
        nv_redirect_location(NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
    }
} else {
