...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    $query = 'SELECT status FROM ' . TABLE . '_unit_weight WHERE id=' . $id;
    $row = $db->query($query)->fetch();
    if (isset($row['status']))     {
        $status = ($row['status']) ? 0 : 1;
        $query = 'UPDATE ' . TABLE . '_unit_weight SET status=' . intval($status) . ' WHERE id=' . $id;
        $db->query($query);
        $content = 'OK_' . $id;
    }
...
if ($nv_Request->isset_request('ajax_action', 'post')) {
    ...
    $content = 'NO_' . $id;
    if ($new_vid > 0)     {
        $sql = 'SELECT id FROM ' . TABLE . '_unit_weight WHERE id!=' . $id . ' ORDER BY weight ASC';
        $result = $db->query($sql);
        $weight = 0;
        while ($row = $result->fetch())
        {
            ++$weight;
            if ($weight == $new_vid) ++$weight;             $sql = 'UPDATE ' . TABLE . '_unit_weight SET weight=' . $weight . ' WHERE id=' . $row['id'];
            $db->query($sql);
        }
        $sql = 'UPDATE ' . TABLE . '_unit_weight SET weight=' . $new_vid . ' WHERE id=' . $id;
        $db->query($sql);
        $content = 'OK_' . $id;
    }
    $nv_Cache->delMod($module_name);
...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $weight=0;
        $sql = 'SELECT weight FROM ' . TABLE . '_unit_weight WHERE id =' . $db->quote($id);
        $result = $db->query($sql);
        list($weight) = $result->fetch(3);
        
        $db->query('DELETE FROM ' . TABLE . '_unit_weight  WHERE id = ' . $db->quote($id));
        if ($weight > 0)         {
            $sql = 'SELECT id, weight FROM ' . TABLE . '_unit_weight WHERE weight >' . $weight;
            $result = $db->query($sql);
            while (list($id, $weight) = $result->fetch(3))
            {
                $weight--;
                $db->query('UPDATE ' . TABLE . '_unit_weight SET weight=' . $weight . ' WHERE id=' . intval($id));
            }
        }
        $nv_Cache->delMod($module_name);
...
if ($nv_Request->isset_request('submit', 'post')) {
    $row['name_weight'] = $nv_Request->get_title('name_weight', 'post', '');
    $row['exchange'] = $nv_Request->get_title('exchange', 'post', '');
	$row['symbol'] = $nv_Request->get_title('symbol', 'post', '');

    if (empty($row['name_weight'])) {
        $error[] = $lang_module['error_required_name_weight'];
    ...
    if (empty($error)) {
        try {
            if (empty($row['id'])) {
                ...
                $stmt = $db->prepare('INSERT INTO ' . TABLE . '_unit_weight (name_weight, exchange, time_add, user_add, status, weight,symbol) VALUES (:name_weight, :exchange, :time_add, :user_add, :status, :weight,:symbol)');
                ...
                $weight = $db->query('SELECT max(weight) FROM ' . TABLE . '_unit_weight')->fetchColumn();
                $weight = intval($weight) + 1;
                $stmt->bindParam(':weight', $weight, PDO::PARAM_INT);


            } else {
                $stmt = $db->prepare('UPDATE ' . TABLE . '_unit_weight SET name_weight = :name_weight, exchange = :exchange , symbol = :symbol, user_edit='.$admin_info['userid'].', time_edit='.NV_CURRENTTIME.' WHERE id=' . $row['id']);
            }
            $stmt->bindParam(':name_weight', $row['name_weight'], PDO::PARAM_STR);
    ...
    $row = $db->query('SELECT * FROM ' . TABLE . '_unit_weight WHERE id=' . $row['id'])->fetch();
    if (empty($row)) {
        nv_redirect_location(NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
    }
} else {
...
$xtpl = new XTemplate('unit_weight.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
		...
		$view['exchange']='1 '.$view['symbol'].' = '.number_format($view['exchange']).' gram';
...
$page_title = $lang_module['unit_weight'];
