 ...
 * @Createdate Wed, 23 Dec 2020 05:00:46 GMT
 */
...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    $query = 'SELECT status FROM ' . TABLE . '_unit WHERE id=' . $id;
    $row = $db->query($query)->fetch();
    if (isset($row['status']))     {
        $status = ($row['status']) ? 0 : 1;
        $query = 'UPDATE ' . TABLE . '_unit SET status=' . intval($status) . ' WHERE id=' . $id;
        $db->query($query);
        $content = 'OK_' . $id;
    }
...
if ($nv_Request->isset_request('ajax_action', 'post')) {
    ...
    $content = 'NO_' . $id;
    if ($new_vid > 0)     {
        $sql = 'SELECT id FROM ' . TABLE . '_unit WHERE id!=' . $id . ' ORDER BY weight ASC';
        $result = $db->query($sql);
        $weight = 0;
        while ($row = $result->fetch())
        {
            ++$weight;
            if ($weight == $new_vid) ++$weight;             $sql = 'UPDATE ' . TABLE . '_unit SET weight=' . $weight . ' WHERE id=' . $row['id'];
            $db->query($sql);
        }
        $sql = 'UPDATE ' . TABLE . '_unit SET weight=' . $new_vid . ' WHERE id=' . $id;
        $db->query($sql);
        $content = 'OK_' . $id;
    }
    $nv_Cache->delMod($module_name);
...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $weight=0;
        $sql = 'SELECT weight FROM ' . TABLE . '_unit WHERE id =' . $db->quote($id);
        $result = $db->query($sql);
        list($weight) = $result->fetch(3);
        
        $db->query('DELETE FROM ' . TABLE . '_unit  WHERE id = ' . $db->quote($id));
        if ($weight > 0)         {
            $sql = 'SELECT id, weight FROM ' . TABLE . '_unit WHERE weight >' . $weight;
            $result = $db->query($sql);
            while (list($id, $weight) = $result->fetch(3))
            {
                $weight--;
                $db->query('UPDATE ' . TABLE . '_unit SET weight=' . $weight . ' WHERE id=' . intval($id));
            }
        }
        $nv_Cache->delMod($module_name);
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete Unit', 'ID: ' . $id, $admin_info['userid']);
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($error)) {
        try {
            if (empty($row['id'])) {
                ...
                $stmt = $db->prepare('INSERT INTO ' . TABLE . '_unit (name, time_add, user_add, weight, status) VALUES (:name, :time_add, :user_add,:weight, :status)');

                $stmt->bindParam(':time_add', $row['time_add'], PDO::PARAM_INT);
                $stmt->bindParam(':user_add', $row['user_add'], PDO::PARAM_INT);
               
                $weight = $db->query('SELECT max(weight) FROM ' . TABLE . '_unit')->fetchColumn();
                ...
                $stmt = $db->prepare('UPDATE ' . TABLE . '_unit SET name = :name, user_edit='.NV_CURRENTTIME.',time_edit='.$admin_info['userid'].' WHERE id=' . $row['id']);
            }
            ...
            if ($exc) {
                ...
                if (empty($row['id'])) {
                    ...
                    nv_insert_logs(NV_LANG_DATA, $module_name, 'Edit Unit', 'ID: ' . $row['id'], $admin_info['userid']);
                }
                nv_redirect_location(NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
            }
    ...
    $row = $db->query('SELECT * FROM ' . TABLE . '_unit WHERE id=' . $row['id'])->fetch();
    if (empty($row)) {
        nv_redirect_location(NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
    }
} else {
