 ...
 * @Createdate Wed, 23 Dec 2020 07:43:38 GMT
 */
...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    $query = 'SELECT status FROM ' . TABLE . '_tabs WHERE id=' . $id;
    $row = $db->query($query)->fetch();
    if (isset($row['status']))     {
        $status = ($row['status']) ? 0 : 1;
        $query = 'UPDATE ' . TABLE . '_tabs SET status=' . intval($status) . ' WHERE id=' . $id;
        $db->query($query);
        $content = 'OK_' . $id;
    }
...
if ($nv_Request->isset_request('ajax_action', 'post')) {
    ...
    $content = 'NO_' . $id;
    if ($new_vid > 0)     {
        $sql = 'SELECT id FROM ' . TABLE . '_tabs WHERE id!=' . $id . ' ORDER BY weight ASC';
        $result = $db->query($sql);
        $weight = 0;
        while ($row = $result->fetch())
        {
            ++$weight;
            if ($weight == $new_vid) ++$weight;             $sql = 'UPDATE ' . TABLE . '_tabs SET weight=' . $weight . ' WHERE id=' . $row['id'];
            $db->query($sql);
        }
        $sql = 'UPDATE ' . TABLE . '_tabs SET weight=' . $new_vid . ' WHERE id=' . $id;
        $db->query($sql);
        $content = 'OK_' . $id;
    }
    $nv_Cache->delMod($module_name);
...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $weight=0;
        $sql = 'SELECT weight FROM ' . TABLE . '_tabs WHERE id =' . $db->quote($id);
        $result = $db->query($sql);
        list($weight) = $result->fetch(3);
        
        $db->query('DELETE FROM ' . TABLE . '_tabs  WHERE id = ' . $db->quote($id));
        if ($weight > 0)         {
            $sql = 'SELECT id, weight FROM ' . TABLE . '_tabs WHERE weight >' . $weight;
            $result = $db->query($sql);
            while (list($id, $weight) = $result->fetch(3))
            {
                $weight--;
                $db->query('UPDATE ' . TABLE . '_tabs SET weight=' . $weight . ' WHERE id=' . intval($id));
            }
        }
        $nv_Cache->delMod($module_name);
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete Tabs', 'ID: ' . $id, $admin_info['userid']);
...
if ($nv_Request->isset_request('submit', 'post')) {
    $row['name_tabs'] = $nv_Request->get_title('name_tabs', 'post', '');
    $row['image'] = $nv_Request->get_title('image', 'post', '');
    if (is_file(NV_DOCUMENT_ROOT . $row['image']))     {
        $row['image'] = substr($row['image'], strlen(NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/tab/'));
    } else {
        $row['image'] = '';
    }
    $row['content_id'] = $nv_Request->get_int('content_id', 'post', 0);

    if (empty($row['name_tabs'])) {
        $error[] = $lang_module['error_required_name_tabs'];
    }
    ...
    if (empty($error)) {
        try {
            if (empty($row['id'])) {
                ...
                $stmt = $db->prepare('INSERT INTO ' . TABLE . '_tabs (name_tabs, image, content_id, time_add, user_add, weight, status) VALUES (:name_tabs, :image, :content_id, :time_add, :user_add, :weight, :status)');

                $stmt->bindParam(':time_add', $row['time_add'], PDO::PARAM_INT);
                $stmt->bindParam(':user_add', $row['user_add'], PDO::PARAM_INT);
              
                $weight = $db->query('SELECT max(weight) FROM ' . TABLE . '_tabs')->fetchColumn();
                ...
                $stmt = $db->prepare('UPDATE ' . TABLE . '_tabs SET name_tabs = :name_tabs, image = :image, content_id = :content_id, user_edit='.$admin_info['userid'].',time_edit='.NV_CURRENTTIME.' WHERE id=' . $row['id']);
            }
            $stmt->bindParam(':name_tabs', $row['name_tabs'], PDO::PARAM_STR);
            $stmt->bindParam(':image', $row['image'], PDO::PARAM_STR);
            $stmt->bindParam(':content_id', $row['content_id'], PDO::PARAM_INT);

            $exc = $stmt->execute();
            if ($exc) {
                $nv_Cache->delMod($module_name);
                if (empty($row['id'])) {
                    nv_insert_logs(NV_LANG_DATA, $module_name, 'Add Tabs', ' ', $admin_info['userid']);
                } else {
                    nv_insert_logs(NV_LANG_DATA, $module_name, 'Edit Tabs', 'ID: ' . $row['id'], $admin_info['userid']);
                }
                nv_redirect_location(NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
            }
    ...
    $row = $db->query('SELECT * FROM ' . TABLE . '_tabs WHERE id=' . $row['id'])->fetch();
    if (empty($row)) {
        nv_redirect_location(NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
    }
} else {
...
if (!empty($row['image']) and is_file(NV_UPLOADS_REAL_DIR . '/' . $module_upload . '/' . $row['image'])) {
    $row['image'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/tab/' . $row['image'];
}
...
$xtpl->assign('Q', $q);
$tabs = array();
    $tabs[] = array( 'id'=>1, 'text'=>'Chi tiết sản phẩm' );
    $tabs[] = array( 'id'=>2, 'text'=>'Bình luận' );
    $tabs[] = array( 'id'=>3, 'text'=>'Đánh giá sản phẩm' );

foreach ( $tabs as $filt_tabs ) {
	$xtpl->assign('TAB', array(
                'key' => $filt_tabs['id'],
                'title' => $filt_tabs['text'],
                'selected' => ($filt_tabs['id'] == $row['content_id']) ? ' selected="selected"' : ''));
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
		...
		if($view['content_id']==1){
			$view['content_id']='Chi tiết sản phẩm';
		}else if($view['content_id']==2){
			$view['content_id']='Bình luận';
		}else if($view['content_id']==3){
			$view['content_id']='Đánh giá sản phẩm';
		}
		if (!empty($view['image']) and is_file(NV_UPLOADS_REAL_DIR . '/' . $module_upload . '/tab/' . $view['image'])) {
			$view['image'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/tab/' . $view['image'];
		}else{
