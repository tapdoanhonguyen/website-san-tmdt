...
function nv_site_theme($contents, $full = true)
{
    ...
    // Header variables
    $xtpl->assign('LINK_LOGIN', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=users&amp;' . NV_OP_VARIABLE . '=login');
    $xtpl->assign('LINK_REGISTER', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=users&amp;' . NV_OP_VARIABLE . '=register');
    ...
    if ($full) {
        ...
        if (!$home and $op != 'product-shock') {
			...
			if ($global_config['rewrite_op_mod'] == $module_name) {
				...
				if(end($array_mod_title_copy)['title'] == ''){
					array_pop($array_mod_title_copy);
				}
            }
        ...
        // Change theme types
        if (sizeof($global_config['array_theme_type']) > 1 and defined('NV_IS_ADMIN')) {
