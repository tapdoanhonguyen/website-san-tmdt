    ...
    <div class="row">
        <div class="col-md-5 m-3 d-flex">
            ...
            <div class="productDetail_shop_info ml-3 mt-3">
                ...
                <input type="hidden" name="check_{info_shop.id}" value="{FOLLOW.check_follow}">
            </div>
        </div>
                ...
                if (check == 0) {
                    $.ajax({
                        ...
                        url: nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' +
                            nv_fc_variable + '=viewcatshops&mod=follow',
                        beforeSend: function() {
                            $('#follow_shop_' + shop_id).prop('disabled', true);
                        },
                        complete: function() {
                            $('#follow_shop_' + shop_id).prop('disabled', false);
                        },
                    ...
                    $.ajax({
                        ...
                        dataType: "JSON",
                        url: nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' +
                            nv_fc_variable + '=viewcatshops&mod=un_follow',
                        beforeSend: function() {
                            $('#follow_shop_' + shop_id).prop('disabled', true);
                        },
                        complete: function() {
...
<script>
    function save_voucher(voucher_id) {
        $.ajax({
            ...
            beforeSend: function() {
                $('#voucher_' + voucher_id).prop('disabled', true);
            },
            complete: function() {
                $('#voucher_' + voucher_id).prop('disabled', false);
            },
