				...
				<table class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<!-- END: weightshowlang -->
							<td align="left">{LANG.street_name}</td>
	...
	if( confirm( '{LANG.confirm}' )) 
	{
		$.ajax({
			...
			success: function(json) {
				...
				if (json['error']) {
					$('#content').prepend('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + '<i class="fa fa-times"></i></div>');
				}
				
				if (json['success']) {
					$('#content').prepend('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + '<i class="fa fa-times"></i></div>');
