...
<div class="panel panel-default">
    ...
    <h3 class="panel-title" style="float:left"><i class="fa fa-list"></i> {LANG.category}</h3> 
    <div class="pull-right">
	 ...
	 <a href="{category_add}" data-toggle="tooltip" data-placement="top" title="{LANG.category_add}" class="btn btn-success btn-sm"><i class="fa fa-plus-circle" aria-hidden="true"></i></a>
   </div>
        ...
        <table class="table table-striped table-bordered table-hover">
            <thead>
                <tr>
                    ...
                    <th class="text-center">{LANG.name_category}</th>
					<th class="text-center">{LANG.inhome}</th>
                    <th class="text-center">{LANG.viewcat}</th>
            ...
            <tbody>
                ...
                <tr class="text-center">
					...
					<td align="left">
						<select class="form-control" id="id_viewcat_{VIEW.id}" onchange="nv_chang_cat('{VIEW.id}','viewcat');">
							<!-- BEGIN: viewcat -->
...
//<![CDATA[
    function nv_change_weight(id) {
        ...
        $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=category&nocache=' + new Date().getTime(), 'ajax_action=1&id=' + id + '&new_vid=' + new_vid, function(res) {
            var r_split = res.split('_');
            if (r_split[0] != 'OK') {
                alert(nv_is_change_act_confirm[2]);
            }
            window.location.href = script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=category';
            return;
        });
        return;
    }
    ...
    function nv_change_status(id) {
        ...
        if (confirm(nv_is_change_act_confirm[0])) {
            ...
            $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=category&nocache=' + new Date().getTime(), 'change_status=1&id='+id, function(res) {
                var r_split = res.split('_');
                if (r_split[0] != 'OK') {
                    alert(nv_is_change_act_confirm[2]);
                }
