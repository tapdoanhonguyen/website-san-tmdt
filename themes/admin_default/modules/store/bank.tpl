    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <select class="form-control" id="id_weight_{VIEW.bank_id}" onchange="nv_change_weight('{VIEW.bank_id}');">
                        <!-- BEGIN: weight_loop -->
                            <option value="{WEIGHT.key}"{WEIGHT.selected}>{WEIGHT.title}</option>
                        <!-- END: weight_loop -->
                    </select>
                </td>
                    <td> {VIEW.bank_code} </td>
                    <td> {VIEW.name_bank} </td>
                    <td> {VIEW.user_add} </td>
                    <td> {VIEW.time_add} </td>
                    <td> {VIEW.user_edit} </td>
                    <td> {VIEW.time_edit} </td>
                    <td class="text-center"><input type="checkbox" name="status" id="change_status_{VIEW.bank_id}" value="{VIEW.bank_id}" {CHECK} onclick="nv_change_status({VIEW.bank_id});" /></td>
...
<form class="form-horizontal" action="{NV_BASE_ADMINURL}index.php?{NV_LANG_VARIABLE}={NV_LANG_DATA}&amp;{NV_NAME_VARIABLE}={MODULE_NAME}&amp;{NV_OP_VARIABLE}={OP}" method="post">
    ...
    <div class="form-group">
        <label class="col-sm-5 col-md-4 control-label"><strong>{LANG.bank_code}</strong> <span class="red">(*)</span></label>
        <div class="col-sm-19 col-md-20">
            <input class="form-control" type="text" name="bank_code" value="{ROW.bank_code}" required="required" oninvalid="setCustomValidity(nv_required)" oninput="setCustomValidity('')" />
        </div>
    </div>
    <div class="form-group">
        <label class="col-sm-5 col-md-4 control-label"><strong>{LANG.name_bank}</strong> <span class="red">(*)</span></label>
        <div class="col-sm-19 col-md-20">
            <input class="form-control" type="text" name="name_bank" value="{ROW.name_bank}" required="required" oninvalid="setCustomValidity(nv_required)" oninput="setCustomValidity('')" />
        </div>
    </div>
...
//<![CDATA[
    function nv_change_weight(id) {
        ...
        $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=bank&nocache=' + new Date().getTime(), 'ajax_action=1&bank_id=' + id + '&new_vid=' + new_vid, function(res) {
    ...
    function nv_change_status(id) {
        ...
        if (confirm(nv_is_change_act_confirm[0])) {
            ...
            $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=bank&nocache=' + new Date().getTime(), 'change_status=1&bank_id='+id, function(res) {
                var r_split = res.split('_');
                if (r_split[0] != 'OK') {
                    alert(nv_is_change_act_confirm[2]);
                }
