...
<div class="panel panel-default">
    ...
    <h3 class="panel-title" style="float:left"><i class="fa fa-list"></i> {LANG.tabs}</h3> 
        ...
        <table class="table table-striped table-bordered table-hover">
            <thead>
                <tr>
                    ...
                    <th class="text-center">{LANG.name_tabs}</th>
                    <th class="text-center">{LANG.image}</th>
                    <th class="text-center">{LANG.content_id}</th>
...
<form class="form-horizontal" action="{NV_BASE_ADMINURL}index.php?{NV_LANG_VARIABLE}={NV_LANG_DATA}&amp;{NV_NAME_VARIABLE}={MODULE_NAME}&amp;{NV_OP_VARIABLE}={OP}" method="post">
    ...
    <div class="form-group">
        <label class="col-sm-5 col-md-4 control-label"><strong>{LANG.name_tabs}</strong> <span class="red">(*)</span></label>
        <div class="col-sm-19 col-md-20">
            <input class="form-control" type="text" name="name_tabs" value="{ROW.name_tabs}" required="required" oninvalid="setCustomValidity(nv_required)" oninput="setCustomValidity('')" />
        </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        <label class="col-sm-5 col-md-4 control-label"><strong>{LANG.content_id}</strong></label>
        <div class="col-sm-19 col-md-20">
             <select class="form-control" name="content_id">
               <!-- BEGIN: tabs -->
                 <option value="{TAB.key}" {TAB.selected}>{TAB.title}</option>
               <!-- END: tabs -->
			</select>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $(".selectfile").click(function() {
        ...
        var currentpath = "{NV_UPLOADS_DIR}/{MODULE_UPLOAD}/tab";
        var type = "image";
