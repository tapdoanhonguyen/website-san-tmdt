...
<div class="panel panel-default">
    <div class="panel-body">
        <form class="form-horizontal" action="{NV_BASE_ADMINURL}index.php?{NV_LANG_VARIABLE}={NV_LANG_DATA}&amp;{NV_NAME_VARIABLE}={MODULE_NAME}&amp;{NV_OP_VARIABLE}={OP}" method="post">
            ...
            <div class="form-group">
                <label class="col-sm-5 col-md-4 control-label"><strong>Tên xuất xứ</strong> <span class="red">(*)</span></label>
...
function nv_get_alias(id) {
    ...
    if (title != '') {
        $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=origin&nocache=' + new Date().getTime(), 'get_alias_title=' + encodeURIComponent(title), function(res) {
...
function nv_change_weight(id) {
    ...
    $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=origin&nocache=' + new Date().getTime(), 'ajax_action=1&id=' + id + '&new_vid=' + new_vid, function(res) {
        var r_split = res.split('_');
        if (r_split[0] != 'OK') {
            alert(nv_is_change_act_confirm[2]);
        }
        window.location.href = script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=origin';
        return;
    });
    return;
}
...
function nv_change_status(id) {
    ...
    if (confirm(nv_is_change_act_confirm[0])) {
        ...
        $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=origin&nocache=' + new Date().getTime(), 'change_status=1&id='+id, function(res) {
            var r_split = res.split('_');
            if (r_split[0] != 'OK') {
                alert(nv_is_change_act_confirm[2]);
            }
