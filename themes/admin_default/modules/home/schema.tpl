        ...
        <table class="table table-striped table-bordered table-hover">
            ...
            <tbody>
                ...
                <td>
                    ...
                    <td> <a href="{VIEW.add_content}" title="Thêm nội dung schema">{VIEW.title}</a>
					<br/>{VIEW.item}
...
//<![CDATA[
    function nv_change_weight(id) {
        ...
        $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=schema&nocache=' + new Date().getTime(), 'ajax_action=1&bid=' + id + '&new_vid=' + new_vid, function(res) {
            var r_split = res.split('_');
            if (r_split[0] != 'OK') {
                alert(nv_is_change_act_confirm[2]);
            }
            window.location.href = script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=schema';
            return;
        });
        return;
    }
    ...
    function nv_change_status(id) {
        ...
        if (confirm(nv_is_change_act_confirm[0])) {
            ...
            $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=schema&nocache=' + new Date().getTime(), 'change_status=1&bid='+id, function(res) {
                var r_split = res.split('_');
                if (r_split[0] != 'OK') {
                    alert(nv_is_change_act_confirm[2]);
                }
