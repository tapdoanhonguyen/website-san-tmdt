<!-- BEGIN: main -->
<div class="catalogys_list_export">
	<div class="row">
		<div class="col-md-6">
			<select class="form-control" name="catalogy">
				<option value="0">-- Chọn ngành hàng --</option>
				<!-- BEGIN: catalogy -->
				<option value="{catalogy.id}">{catalogy.name}</option>
				<!-- END: catalogy -->
			</select>
		</div>
		<div class="col-md-6">
...
<div class="mt-4">
	<form id="searchForm" method="post" enctype="multipart/form-data">
		...
		<input class="btn_ecng" type="button" id="submit" name="submit" value="Đăng hàng loạt" />
	</form>
</div>
<div class="row mt-2">
...
<script>
	...
	$('#submit').on('click', function (){
		if($(".file_excel").get(0).files.length == 0){
			...
			$(".export_result").css({"color": "blue", "cursor": "pointer"}).bind("click", function(e){
				...
				$.ajax({               
					type: "GET",
					url: nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=product-import&mod=is_download_result',
					dataType: 'json',              
			...
			$.ajax({
				url : nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=product-import&mod=handle-data',
				type : 'post',
				data : formData,
				dataType: 'json',
				cache: false,
	...
	function sendAjax(index, data, numok, numerr){
		...
		$.ajax({
			url : nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=product-import&mod=submit',
			type : 'post',
			dataType: 'json',
	...
	$(".export_file").on('click', function(e) 
	{
		...
		e.preventDefault();  
		var catalogy = $('select[name=catalogy]').val();
		if(catalogy == 0)
		{
		...
		$.ajax({               
			type: "GET",
			url: nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=product-import&mod=is_download',
			data: {catalogy : catalogy},
			dataType: 'json',             
			success: function(res) {
				//console.log(res);
				if(res.status == 'OK')
