...
<div class="content_products_item">
	<h4 class="fs_20">Danh sách sản phẩm</h4>
            ...
            <div class="row  p-3 rounded">
                ...
                <div class="col-3">
                    <div class="p-1 rounded rounded-lg border d-flex">
                        ...
                        <input id="ngaytu" name="ngay_tu" value="{ngay_tu}" type="search" placeholder="Từ ngày" aria-describedby="button-addon2" class="form-control border-0 " />
					</div>
				</div>
                <div class="col-3">
                    <div class="p-1 rounded rounded-lg border d-flex">
                        ...
                        <input id="ngayden" name="ngay_den" value="{ngay_den}" type="search" placeholder="Đến ngày" aria-describedby="button-addon2" class="form-control border-0" />
					</div>
				</div>
                <div class="col-3">
                    <div class="p-1 rounded rounded-lg border d-flex">
                ...
                <div class="col-3 d-flex align-items-center">
                    <button class="btn_ecng mr-3">Tìm kiếm</button>
                    <button class="btn_ecng_outline export_file">Xuất Excel</button>
				</div>
			</div>
		</form>
		<div class="tab_menu px-3">
			<ul class="nav nav-pills d-flex" role="tablist">
				...
				<li class="nav-item tab_active4 py-3">
					<a status_id="0" onclick="get_product(0);" class=" py-3 px-4" href="">Đã ẩn</a>
				</li>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div class="cart-item__cell-checkbox py-3 ml-4 d-flex align-items-center">
			...
			<div class="px-3">
				<select class="option_product p-2 rounded">
					<option value="0" option_product="0">Lựa chọn</option>
					<option value="1" option_product="1">FreeShip hàng loạt11</option>
			...
			<a class="btn_ecng" onclick="check_option()">Thực hiện</a>
		</div>
        <div><a href="{product_add}" class="btn_ecng">+ Thêm sản phẩm </a></div>
	</div>
    ...
    <div class="manage_product bg_white rounded">
		...
		<div class="px-3">
			<select class="option_product p-2 rounded">
				<option value="0" option_product="0">Lựa chọn</option>
				<option value="1" option_product="1">FreeShip hàng loạt123</option>
				<option value="2" option_product="2">Hủy FreeShip hàng loạt</option>
				<option value="3" option_product="3">Tự vận chuyển</option>
				<option value="4" option_product="4">Hủy tự vận chuyển</option>
				<option value="6" option_product="6">Tắt trạng thái</option>
...
<script type="text/javascript">
	...
	$('.nav-item a').on('click', function () {
		...
		var $el = $(this);
		if($el.data('id') == 2){
			$(".cart-item__cell-checkbox").css("visibility","hidden");
		}
		else{
			$(".cart-item__cell-checkbox").css("visibility","inherit");
		}
	});
	...
	$("form#formsearch").on('submit', function(e) {
		...
		$.ajax({               
			type: "GET", 
			url: nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=product&mod=tab_product_status',
	...
	get_product({index_active});
	
	function get_product(status)
	{	
		
		$.ajax({               
			type: "GET", 
			url: nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=product1&mod=tab_product_status',
			data: {
				status:status,
			},
			beforeSend: function() {
	...
	function nv_change_status(id) {
        ...
        if (confirm(nv_is_change_act_confirm[0])) {
            ...
            $.post(nv_base_siteurl + "index.php" + "?" + nv_name_variable + "=" + nv_module_name + "&" + nv_fc_variable + "=product&nocache=" + new Date().getTime(), "change_status=1&id=" + id, function (res) {
	...
	function change_freeship(id) {
		...
		if (confirm(nv_is_change_act_confirm[0])) {
            ...
            $.post(nv_base_siteurl + "index.php" + "?" + nv_name_variable + "=" + nv_module_name + "&" + nv_fc_variable + "=product&nocache=" + new Date().getTime(), "change_freeship=1&id=" + id, function (res) {
                var r_split = res.split("_");
                if (r_split[0] != "OK") {
                    alert(nv_is_change_act_confirm[2]);
				}
	...
	function nv_change_freeship(id) {
		...
		var new_freeship = $('#change_freeship_'+ id);
		
		if( new_freeship.is(':checked')) {
			
			freeship_arr.push(id);
			console.log(freeship_arr);
	...
	function check_option(){
		...
		if( option_product == 1){
			...
			$.ajax({
				type : 'GET',
				url : nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=ajax&mod=freeship',
			...
			$.ajax({
				type : 'GET',
				url : nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=ajax&mod=unfreeship',
			...
			$.ajax({
				type : 'GET',
				url : nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=ajax&mod=self_transport',
			...
			$.ajax({
				type : 'GET',
				url : nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=ajax&mod=un_transport',
			...
			if (confirm('Bạn có muốn xóa sản phẩm đã chọn?')) {
				$.ajax({
					type : 'GET',
					url : nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=ajax&mod=delete_all',
			...
			if (confirm('Bạn có thực sự muốn tắt trạng thái đã chọn')) {
				$.ajax({
					type : 'GET',
					url : nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=ajax&mod=off_product',
			...
			if (confirm('Bạn có thực sự muốn bật trạng thái đã chọn')) {
				$.ajax({
					type : 'GET',
					url : nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=ajax&mod=on_product',
